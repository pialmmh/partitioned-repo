...
public class SmsMultiTableHourlyPartitionTest {
    ...
    private static SplitVerseRepository<SmsRecord, String> repository;
    ...
    @Table(name = "sms")
    public static class SmsRecord implements ShardingEntity<String> {
        ...
        @Override
        public String getPartitionColValue() {
            return createdAt != null ? createdAt.toString() : null;
        }
        @Override
        public void setPartitionColValue(String value) {
            this.createdAt = value != null ? LocalDateTime.parse(value) : null;
        }
    ...
    static void setup() throws Exception {
        ...
        repository = SplitVerseRepository.<SmsRecord, String>builder()
    ...
    void testFindByIdAndPartitionColRange() throws Exception {
        ...
        if (!allEntities.isEmpty()) {
            // Test with exact timestamp (no offset)
            Map.Entry<String, SmsRecord> exactTest = allEntities.get(0);
            @SuppressWarnings("unchecked")
            SplitVerseRepository rawRepo2 = repository;
            SmsRecord exactResult = (SmsRecord) rawRepo2.findByIdAndPartitionColRange(
                exactTest.getKey(),
                exactTest.getValue().getCreatedAt(),
                exactTest.getValue().getCreatedAt().plusNanos(1)
            );
            assertNotNull(exactResult, "Should find SMS with exact timestamp range");
            System.out.println("✓ Found SMS with exact timestamp range");

            // Test with very wide range (entire test period)
            Map.Entry<String, SmsRecord> wideTest = allEntities.get(1);
            @SuppressWarnings("unchecked")
            SplitVerseRepository rawRepo3 = repository;
            SmsRecord wideResult = (SmsRecord) rawRepo3.findByIdAndPartitionColRange(
                wideTest.getKey(),
                baseTime.minusDays(30),
                baseTime.plusDays(30)
            );
            assertNotNull(wideResult, "Should find SMS with very wide date range");
            System.out.println("✓ Found SMS with wide date range");
        }
